...
export default function Consents({ user }: Props) {
  ...
  return (
    <div>
      ...
      {error && (
        <div className="flex items-start gap-1.5 text-xs text-red-700 bg-red-50 dark:bg-red-900/30 dark:text-red-400 rounded-lg px-2.5 py-1.5 mb-2">
          <AlertCircle className="w-3.5 h-3.5 mt-0.5 shrink-0" />
          <span>{error}</span>
        </div>
      )}

      {loading ? <p className="text-xs text-gray-400 dark:text-gray-500 py-2">Cargando…</p>
        : items.length === 0 ? (
...
function ConsentRow({ consent: c, onPrint, onDelete }: {
  ...
  if (c.status === 'pending') {
    return (
      <div className="border border-amber-100 bg-amber-50/60 dark:border-amber-800 dark:bg-amber-900/20 rounded-lg p-2">
        <div className="flex items-center gap-2">
          <div className="shrink-0 p-1.5 bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300 rounded"><Clock className="w-4 h-4" /></div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-800 dark:text-gray-100 truncate">{c.title}</p>
            <p className="text-[10px] text-amber-700 dark:text-amber-400">Pendiente de firma remota</p>
          </div>
          <button onClick={onDelete} title="Cancelar" className="p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/40 rounded">
            <Trash2 className="w-3.5 h-3.5" />
          </button>
        </div>
        {link && (
          <div className="flex items-center gap-1.5 mt-2 pt-2 border-t border-amber-100 dark:border-amber-800">
            <button onClick={copyLink} className="flex items-center gap-1 text-[11px] font-medium text-amber-800 dark:text-amber-300 hover:underline">
  ...
  return (
    <div className="flex items-center gap-2 border border-gray-100 dark:border-gray-700 rounded-lg p-2 bg-white dark:bg-gray-800">
      <div className="shrink-0 p-1.5 bg-blue-50 text-blue-600 dark:bg-blue-900/40 dark:text-blue-300 rounded"><FileSignature className="w-4 h-4" /></div>
      ...
      <button onClick={() => onPrint(c)} title="Ver / imprimir"
        className="p-1 text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/40 rounded">
        <Printer className="w-3.5 h-3.5" />
      </button>
      <button onClick={onDelete} title="Eliminar"
        className="p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/40 rounded">
        <Trash2 className="w-3.5 h-3.5" />
      </button>
    </div>
  );
  ...
  return (
    <Modal title="Firmar consentimiento" onClose={onClose}>
      <form onSubmit={submit} className="space-y-3">
        ...
        <div>
          <label className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">Plantilla *</label>
          <div className="flex gap-2">
            ...
            <button type="button" onClick={viewTemplate} disabled={!templateId}
              className="shrink-0 px-3 text-xs font-medium text-blue-600 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/40 disabled:opacity-40">
              Ver
            </button>
          </div>
          {templates.length === 0 && (
            <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">No hay plantillas con PDF cargado. Súbelas en Ajustes.</p>
          )}
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">Firmante *</label>
            <input required className="input" value={signerName} onChange={e => setSignerName(e.target.value)} />
          </div>
          <div>
            <label className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">Documento</label>
            <input className="input" value={signerDocument} onChange={e => setSignerDocument(e.target.value)} />
          </div>
        </div>

        <div>
          <label className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">Firma *</label>
    ...
    <Modal title="Enviar enlace de firma" onClose={onClose}>
      {link ? (
        <div className="space-y-3">
          <p className="text-sm text-gray-600 dark:text-gray-300">Enlace generado. Compártelo con el paciente para que firme desde su celular.</p>
          <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700/60 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2">
            <p className="flex-1 text-xs text-gray-700 dark:text-gray-300 truncate">{link}</p>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600">
              Listo
            </button>
          </div>
        </div>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div>
            <label className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">Cita *</label>
            ...
            {appointments.length === 0 && (
              <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Este paciente no tiene citas todavía.</p>
            )}
          </div>
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600">Cancelar</button>
