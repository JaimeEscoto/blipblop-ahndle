...
export default function GlobalSearch({ fullScreen, onClose }: Props) {
  ...
  const Results = () => (
    <div className="py-1">
      ...
      {matchedUsers.length > 0 && (
        <div className="mb-1">
          <p className="px-3 py-1 text-[10px] font-bold text-gray-400 uppercase tracking-wide">Pacientes</p>
          {matchedUsers.map(u => (
            <button key={u.id} onMouseDown={() => goUser(u.id)}
              className="w-full flex items-center gap-2.5 px-3 py-2 text-left hover:bg-blue-50 rounded-lg">
              <UserIcon className="w-4 h-4 text-blue-500 shrink-0" />
              <div className="min-w-0">
                <p className="text-sm font-medium text-gray-800 truncate">{u.name}</p>
                <p className="text-xs text-gray-400 truncate">{u.document_id || u.phone || u.email || '—'}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      {matchedAppointments.length > 0 && (
        <div className="mb-1">
          <p className="px-3 py-1 text-[10px] font-bold text-gray-400 uppercase tracking-wide">Citas</p>
          {matchedAppointments.map(a => (
            <button key={a.id} onMouseDown={() => goAppointments(a.user_name)}
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-400 truncate">{a.date} {a.time} · Dr. {a.doctor_name}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      {matchedInvoices.length > 0 && (
        <div>
          <p className="px-3 py-1 text-[10px] font-bold text-gray-400 uppercase tracking-wide">Facturas</p>
          {matchedInvoices.map(i => (
            <button key={i.id} onMouseDown={() => goInvoice(i.id)}
              className="w-full flex items-center gap-2.5 px-3 py-2 text-left hover:bg-blue-50 rounded-lg">
              <Receipt className="w-4 h-4 text-green-600 shrink-0" />
              <div className="min-w-0">
                <p className="text-sm font-medium text-gray-800 truncate">#{String(i.number).padStart(4, '0')} · {i.user_name}</p>
                <p className="text-xs text-gray-400 truncate">{i.date}</p>
              </div>
            </button>
          ))}
        </div>
    ...
    return (
      <div className="fixed inset-0 z-50 bg-white flex flex-col">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-100 shrink-0">
          ...
          />
          <button onClick={onClose} className="shrink-0 p-1 text-gray-400 hover:text-gray-700">
            <X className="w-5 h-5" />
          </button>
        </div>
  ...
  return (
    <div className="relative w-full">
      ...
      {focused && active && (
        <div className="absolute left-0 right-0 mt-1.5 bg-white rounded-xl shadow-2xl border border-gray-100 max-h-96 overflow-y-auto z-10">
          <Results />
        </div>
      )}
    </div>
