...
export default function Home() {
  ...
  const fmtDate = (d: string) => {
    const f = new Date(d + 'T00:00:00').toLocaleDateString('es-CO', { weekday: 'short', day: 'numeric', month: 'short' });
  ...
  const waLink = (a: Appointment) => {
    ...
    const fecha = new Date(a.date + 'T00:00:00').toLocaleDateString('es-CO', { weekday: 'long', day: 'numeric', month: 'long' });
    const msg = encodeURIComponent(
      `Hola ${a.user_name}, le recordamos su cita odontológica el ${fecha} a las ${a.time} con el Dr. ${a.doctor_name}.` +
      (url ? ` Puede ver los detalles aquí: ${url}.` : '') +
      ` ¡Le esperamos!`
    );
  ...
  return (
    <div>
      <div className="mb-5">
        ...
        <p className="text-sm text-gray-500">Resumen de la clínica</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
          <div className="flex items-center gap-2 text-amber-600 mb-1">
            ...
            <span className="text-xs font-medium text-gray-500">Stock bajo</span>
          </div>
      ...
      {lowStock.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-sm font-semibold text-gray-800">Productos por agotarse</h2>
          </div>
            ...
            {lowStock.slice(0, 5).map(item => (
              <div key={item.id} className="flex items-center justify-between px-4 py-2.5">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-amber-700">Quedan {item.quantity} {item.unit} (mín. {item.min_quantity})</p>
                </div>
                <span className="shrink-0 text-xs font-bold text-amber-700 bg-amber-100 px-2 py-1 rounded-full">¡Bajo!</span>
              </div>
            ))}
          </div>
          {lowStock.length > 5 && (
            <Link to="/inventario" className="flex items-center justify-end gap-1 mt-2 text-xs text-blue-600 font-medium hover:underline">
              Ver los {lowStock.length} productos <ChevronRight className="w-3.5 h-3.5" />
            </Link>
          )}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-400 text-sm bg-white rounded-xl border border-gray-100">No hay citas próximas</div>
      ) : (
            ...
            return (
              <div key={a.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  {rel && (
                    <span className={`shrink-0 text-xs font-bold px-2 py-1 rounded-full ${rel === 'Hoy' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>{rel}</span>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => generateAppointmentPDF(a)}
                    ...
                    <Download className="w-3.5 h-3.5" /> Descargar invitación
                  </button>
                  {link ? (
                    ...
                    <span className="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium py-2 bg-gray-50 text-gray-400 rounded-lg cursor-not-allowed" title="Paciente sin teléfono registrado">
                      <MessageCircle className="w-3.5 h-3.5" /> Sin teléfono
                    </span>
                  )}
                </div>
              </div>
